...
class BehAuth
{
    ...
    */
    function is_logged_in($redirect = false, $default_redirect = true)
    {
        ...
        $admin = $this->CI->session->userdata('behandlare');
        // print_a($redirect,true);
        if(!$admin)
        {
            //check the cookie
            if(isset($_COOKIE['KogwebmeetingDoc']))
            {
                //the cookie is there, lets log the customer back in.
                $info = $this->aes256Decrypt(base64_decode($_COOKIE['KogwebmeetingDoc']));
            ...
            if ($default_redirect)
            {
                redirect(config_item('behandlare_folder').'/login');
            }
    ...
    */
    function login($personal_id, $password, $remember=false,$login_type=false)
    {
        ...
        {
            $user = array();
            $user['behandlare'] = array();
            $user['behandlare']['id'] = $result['id'];
            $user['behandlare']['name'] = $result['firstname'].' '.$result['lastname'];
            $user['behandlare']['email'] = $result['email'];
            $user['behandlare']['hsaid'] = $result['hsaid'];
            $user['behandlare']['personal_id'] = $result['personal_id'];
            $user['behandlare']['privilege'] = $result['privilege'];
            $user['behandlare']['profile_pic'] = $result['profile_img'];
            ...
            if($remember)
            {
                ...
                $this->generateCookie($loginCred, strtotime('+1 day'));
            }

            if(!$login_type)
            if(config_item('enable_2-way_authentication'))
            {
                $auth = $this->CI->session->userdata('2way_auth');
                // print_a($auth,true);
                if(!$auth)
                {
                ...
                redirect(config_item('behandlare_folder').'/login/check_otp');
                return true;

            }

            
            /* Set Logged in date time */
            $this->CI->db->where('personal_id',$result['personal_id'])->update('users',array('last_logged_in'=>date('Y-m-d H:i:s')));
    ...
    {
        setcookie('KogwebmeetingDoc', $data, $expire, '/', $_SERVER['HTTP_HOST']);
    }
    ...
    */
    ...
    {
        // echo  123;exit;
        $this->CI->session->unset_userdata('behandlare');
    ...
    */
    function reset_password($email)
    {
        ...
        if ($admin)
        {
            ...
            $this->CI->email->subject(config_item('company_name').': Treatment Specialist Password Reset');
            $message =  'Dear '.$admin['username'].'<br>';
            $post_link = site_url('resetpass?authkey='.$auth_key);
    ...
    */
    ...
    {
        $sess_data = $this->CI->session->userdata('behandlare');
        ...
        if(isset($data['hsaid']))
        $sess_data['hsaid'] = $data['hsaid'];
        
        
        
        $this->CI->session->set_userdata('behandlare',$sess_data);
    }
